...
import { CertificationContract, UserRole } from '../types/contracts';

const CONTRACT_ADDRESS = (import.meta.env.VITE_CONTRACT_ADDRESS ?? '').trim();

export const isContractConfigured = (): boolean => Boolean(CONTRACT_ADDRESS);
...
export const initContract = async (): Promise<CertificationContract> => {
  if (!CONTRACT_ADDRESS) {
    throw new Error(
      'Contract not configured (VITE_CONTRACT_ADDRESS). Demo verification works without it at /demo.'
    );
  }
